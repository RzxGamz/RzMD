...
module.exports = async(conn, msg, m, setting) => {
	try {
		...
		const buttonsDefault = [
			{ urlButton: { displayText: `Website`, url : `https://bit.ly/ss-nembak` } },
		...
		switch(command) {
			...
			case prefix+'help':
			    ...
			    conn.sendMessage(from, { caption: teks, location: { jpegThumbnail: fs.readFileSync(setting.pathimg) }, templateButtons: buttonsDefault, footer: 'Rzx Bot Multi Device', mentions: [sender] })
				break
			case prefix+'sc':
			    reply(`Sc free cari yt yaa _^`)
			    break
			case prefix+'runtime':
			...
			case prefix+'donate':
			case prefix+'donasi':
			    reply(`Hi ${pushname}\n\nJika ingin donasi silahkan chat nomor ini nanti akan saya kasih nomor payment nya :)`)
			    break
			case prefix+'owner':
